...
export default function Login() {
  ...
  // Show loading state immediately if we're in the middle of a Google callback
  const isFinalizingGoogle = !!code && !isAuthenticated;
  ...
  useEffect(() => {
    if (code && !hasCalled.current) {
      ...
      console.log("[Login] Starting Google callback...");
      
      handleGoogleCallback(code)
        .then(() => {
          console.log("[Login] Google callback successful");
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-slate-100">
        ...
        {(error || isFinalizingGoogle) && (
          <div className={`px-4 py-3 rounded-lg text-sm ${isFinalizingGoogle ? 'bg-indigo-50 border border-indigo-200 text-indigo-600' : 'bg-red-50 border border-red-200 text-red-600'}`}>
            {isFinalizingGoogle ? (
